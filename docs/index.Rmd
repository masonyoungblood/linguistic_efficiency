...
where $\sum{\pi}$ is the lifetime benefit of the communication system, $C_{L}$ is the cost of learning it, and $\sum{C_{P}}$ is the lifetime cost of producing it [@gruber_etal22]. This framing is consistent with understandings of efficiency in linguistics [@gibson_etal19]: the famous "principle of least effort" can be thought of as a minimization of these costs [@zipf49].
...
Notions of complexity vary widely and are sometimes uncorrelated across levels of analysis [@mikula_etal18; @benedict_najar19]. In the birdsong literature, for example, complexity is usually described at one of three levels: syllables (individual sounds within songs), songs (sequences of syllables), or repertoires (full set of syllables or songs that a bird produces). Syllable and repertoire complexity are approximated with measures of production cost (e.g. frequency bandwidth, number of transitions in pitch [@benedict_najar19; @youngbloodContentBiasCultural2022]) and learning cost (e.g. diversity of unique syllables or songs [@garamszegi_etal05; @soma_garamszegi11]), respectively. Song complexity, on the other hand, is often characterized by measures of hierarchical or combinatorial structure [@kershenbaumAnimalVocalSequences2014; @sainburgParallelsSequentialOrganization2019]. Structured signals are more compressible and learnable [@raviv_etal21; @Verhoef2012; @gibson_etal19], which allows more information to flow through the "bottleneck" of cultural transmission [@kirby17; @gruber_etal22]. The former increases both production and learning costs, whereas the latter decreases learning costs. Both notions of complexity, however, should boost the benefits of communication by expanding the signal space. Syllable complexity allows for greater diversity in syllable types, and song complexity allows those syllable types to be combined into a wider array of song types.
...
Zipf's rank-frequency law predicts that the frequency of an item will be proportional to the inverse of its rank, a relationship that holds for most, if not all, of the world's languages [@piantadosi14]. In this study, I will focus on Mandelbrot's more flexible parameterization of Zipf's rank-frequency law (see [Analysis]) [@Mandelbrot1953; @Mandelbrot1962], which is its most common form in contemporary linguistics [@piantadosi14]. Non-human animal communication systems, including bird and cetacean vocalizations [@allenNetworkAnalysisReveals2019; @Briefer2010; @cody_etal16; @hailman_etal85; @mccowan_etal99] and lizard courtship displays [@martins94], exhibit more redundancy than languages, leading to a convex rank-frequency relationship that is better captured by the Zipf-Mandelbrot distribution. Zipf and Mandelbrot both interpreted this rank-frequency law as resulting from a minimization of production and perception costs [@zipf49; @Mandelbrot1953], and there are models showing that it can be derived from communicative efficiency [@manin09; @salge_etal15; @ferrer-i-cancho16]. However, some of these models assume that signals map to objects or concepts which is not the case in birdsong, and other causes are still debated [@piantadosi14] (e.g. preferential reuse [@simon55], semantic mappings [@allegrini_etal04], byproduct of abbreviation [@corral_serra20]). Even though there is still uncertainty about its causes, the presence of Zipf's rank-frequency law in non-human communication systems has been interpreted as evidence for both communicative efficiency and information content [@genty_byrne10; @kershenbaum_etal21; @mccowan_etal99; @stepanov_etal23].
...
<!-- Larger syllable and song repertoire sizes are associated with open-ended vocal learning [@creanza_etal16; @robinson_etal19], better problem-solving, and larger brain size [@audet_etal23]. -->

<!-- THE most relevant review of CCE for this paper is @buskell22!! Distinguishes "cultural accumulation" into: adaptiveness, complexity, efficiency, and disparity (read as variation or diversity in biological terms). Read in full before adding notes here. -->

<!-- GENERAL definition and discussion of CCE [@mesoudi_thornton18]!! -->

<!-- In humpback whales, songs becomes more complex during periods of stability but then crashes in complexity during song "revolutions". @garland_etal22 claim that this is a tentative case of CCE, but that what is missing is the link between female preferences and complexity (and eventual fitness). -->

<!-- An informal poll at a cultural evolution conference suggests that researchers prefer to think about CCE as increasing "functionality" rather than complexity or efficiency, presumably because of difficulties in defining complexity in a generalizable way [@wilks_blakey18]. In fact, decreased complexity and increased efficiency may be what "lock in" more functional traits in populations [@wilks_blakey18]. -->

<!-- There are huge definitional issues with CCE, especially when applied to non-human animals... what does complexity mean, should we focus on products vs. processes, etc. [@wilks_blakey18; @rawlings_etal21; @sinclair_etal22] -->

<!-- Birdsong, like human aesthetic culture, is constantly being optimized for a moving target: female preferences. The functional link between the trait and the target, though, is at least much more obvious than it is for human aesthetic culture [@sinclair_etal22]. -->

<!-- HUMANS AND LINGUISTICS -->

<!-- A part of communicative efficiency is keeping the information content of spoken and written language constant with respect to time (linkable to bird efficiency stuff?) [@jaeger_tily11]. -->

<!-- Lower complexity may boost learnability by either increasing the overall input or just making it easy to learn [@jaeger_tily11]. -->

<!-- Zipf's laws and Menzerath's law apply to both written words and acoustic signals [@torre_etal19]. -->

<!-- Zipf himself thought that languages evolved to maximize information and minimize cost [@zipf49] -->

<!-- Grammars of languages from around the world have been found to optimize the ratio of information to communicative cost [@hahn_etal20]. -->

<!-- ITERATED LEARNING -->

<!-- It is well-established at this point that, in human symbolic systems, complex signals evolve to become more compressible over the course of cultural transmission [@caldwell_smith12; @tamarizCultureCopyingCompression2015; @tamariz17; @kellyPredictableEvolutionLetter2021]. -->

<!-- Strong social biases can cause artificial languages to become less efficient than they otherwise would be [@fedzechkina_etal23].  -->

<!-- BIRDSONG AND ANIMALS -->

<!-- There are also known trade-offs between complexity at different levels of analysis. In wood warblers, for example, species with faster trills usually have songs with simpler syntax [@cardoso_hu11]. -->

<!-- In some species, complexity in syllable sequence appears to remain constant across songs of different lengths [@palmero_etal14]. -->

<!-- Biophysical trade-offs mean that males are usually only high-performance on a subset of possible syllable features [@goller22]. -->

<!-- Species with a higher baseline metabolic rate also have higher song complexity (i.e. syllable and song repertoire sizes) [@garamszegi_etal06], but the metabolic cost of singing songs with different lengths does not appear to vary within species [@oberweger_goller01]. -->


<!-- In animal communication systems, duration may not be the only cost subject to Zipfian abbreviation. Rock hyraxes, for example, produce low-amplitude calls much more often than high-amplitude calls [@demartsev_etal19]. -->

<!-- -   Efficiency at the level of syllables, using the three most commonly studied linguistic laws recommended by @semple_etal22. -->
<!--     -   Zipf's rank-frequency law: the frequency of an item is approximately the inverse of its rank [@zipf35]. -->
<!--     -   Zipf's law of abbrevation: common items have a lower production cost [@zipf49]. -->
<!--     -   Menzerath's law: longer sequences are comprised of shorter items [@menzerath54]. -->
<!-- -   Structure at the level of sequences. -->
<!--     -   Small-world structure: the network of syllable transitions has high clustering and short path lengths [@watts_strogatz98]. -->
<!--     -   Mutual information decay: the mutual information of syllable pairs at short and long distances are consistent with Markov chains and hierarchical structure, respectively [@sainburgParallelsSequentialOrganization2019]. -->

<!-- RANDOM -->

<!-- Other statistical patterns are thought to be scale invariant, or roughly the same regardless of scale: @gervain_geffen19 @bialek_shaevitz23 @stanley_etal00 @chater_brown99 @gisiger01 @mehri_jamaati21. "Fractal equivalency" [@lahti20] -->

<!-- They also decrease the duration of songs and calls in noisy environments, although the reason for this change is less clear [@bermudez-cuamatzin_etal23]. -->
