...
<br>![](https://www.nps.gov/common/uploads/cropped_image/primary/76E27458-A21E-1339-E0DE16CC712C3FA9.jpg?width=1600&quality=90&mode=crop)
&copy; National Park Service
...
Menzerath's law predicts that longer sequences will be comprised of shorter items to balance production costs [@menzerath54]. This negative correlation between sequence length and item length is found at various levels of analysis in language (e.g. clauses in sentences, morphemes in words) [@cramer05; @stave_etal21; @eroglu13]<!-- as well as in music [@boroda_altmann91]-->. In non-human communication, Menzerath's law appears to be present in chimpanzee gesture [@heesen_etal19] and in the vocalizations of some primates and birds [@gustisonGeladaVocalSequences2016; @huang_etal20; @favaro_etal20; @fedurek_etal17], including house finches [@jamesPhylogenyMechanismsShared2021]<!--, although its detection may depend on the type of vocalization [@clink_lau20] and level of analysis [@clink_etal20]-->. The explanation for Menzerath's law is an extension of Zipf's law of abbreviation: when production costs are increased in one domain (e.g. song sequence length) they should be decreased in another (e.g. syllable duration).
...
```{r unseen-species, echo = FALSE, eval = FALSE}
#load libraries
library(iNEXT)
library(ggplot2)
library(cowplot)

#run unseen species models
unseen_2 <- iNEXT(as.numeric(sort(table(data$cluster_2), decreasing = TRUE)), q = 0)
unseen_3 <- iNEXT(as.numeric(sort(table(data$cluster_3), decreasing = TRUE)), q = 0)
unseen_4 <- iNEXT(as.numeric(sort(table(data$cluster_4), decreasing = TRUE)), q = 0)

#construct rarefaction plots for each level of deep split
unseen_plot_2 <- ggiNEXT(unseen_2) + ggtitle("Deep Split: 2") + 
  xlab("Number of sampled syllables") + ylab("Richness (*q* = 0)") + 
  theme_linedraw(base_size = 8, base_family = "Avenir") + theme(legend.position = "none", axis.title.y = ggtext::element_markdown()) + 
  guides(linetype = "none") + scale_color_manual(values = "#009E73") + scale_fill_manual(values = "#009E73")
unseen_plot_2$layers[[1]]$aes_params$size <- 2
unseen_plot_2$layers[[2]]$aes_params$linewidth <- 0.5
unseen_plot_3 <- ggiNEXT(unseen_3) + ggtitle("Deep Split: 3") + 
  xlab("Number of sampled syllables") + ylab("Richness (*q* = 0)") + 
  theme_linedraw(base_size = 8, base_family = "Avenir") + theme(legend.position = "none", axis.title.y = ggtext::element_markdown()) + 
  guides(linetype = "none") + scale_color_manual(values = "#D55E00") + scale_fill_manual(values = "#D55E00")
unseen_plot_3$layers[[1]]$aes_params$size <- 2
unseen_plot_3$layers[[2]]$aes_params$linewidth <- 0.5
unseen_plot_4 <- ggiNEXT(unseen_4) + ggtitle("Deep Split: 4") + 
  xlab("Number of sampled syllables") + ylab("Richness (*q* = 0)") + 
  theme_linedraw(base_size = 8, base_family = "Avenir") + theme(legend.position = "none", axis.title.y = ggtext::element_markdown()) + 
  guides(linetype = "none") + scale_color_manual(values = "#0072B2") + scale_fill_manual(values = "#0072B2")
unseen_plot_4$layers[[1]]$aes_params$size <- 2
unseen_plot_4$layers[[2]]$aes_params$linewidth <- 0.5

#combined rarefaction plots and save
png("plots/unseen_species.png", width = 6.8, height = 1.8, units = "in", res = 600)
plot_grid(unseen_plot_2, unseen_plot_3, unseen_plot_4, nrow = 1)
dev.off()
```

```{r, echo = FALSE, eval = FALSE}
#compute r-squared for each measure and each level of deep split
token_var_by_type <- do.call(rbind, parallel::mclapply(1:4, function(x){
  #create temporary data frame with corresponding measure of cost
  if(x == 1){temp_data <- data.frame(measure = scale(data$duration))}
  if(x == 2){temp_data <- data.frame(measure = scale(data$bandwidth))}
  if(x == 3){temp_data <- data.frame(measure = scale(data$concavity))}
  if(x == 4){temp_data <- data.frame(measure = scale(data$excursion))}
  
  unlist(lapply(1:3, function(y){
    #add cluster that corresponds to level of deep split
    if(y == 1){temp_data$cluster <- data$cluster_2}
    if(y == 2){temp_data$cluster <- data$cluster_3}
    if(y == 3){temp_data$cluster <- data$cluster_4}
    
    #get r-squared value from model that just includes the cluster
    return(performance::r2(lme4::lmer(scale(measure) ~ (1|cluster), data = temp_data))[1])
  }))
}, mc.cores = 4))

#restructure the matrix for inclusion in the supplement
token_var_by_type <- data.frame(token_var_by_type)
colnames(token_var_by_type) <- c("DS: 2", "DS: 3", "DS: 4")
rownames(token_var_by_type) <- c("Duration", "Bandwidth", "Concavity", "Excursion")

#save
save(token_var_by_type, file = "data_models/token_var_by_type.RData")
```

```{r, echo = FALSE, eval = FALSE}
#compute median values for each individual type
summarized_data <- list(cbind(aggregate(duration ~ cluster_2, data = data, median), 
                              bandwidth = aggregate(bandwidth ~ cluster_2, data = data, median)$bandwidth, 
                              concavity = aggregate(concavity ~ cluster_2, data = data, median)$concavity, 
                              excursion = aggregate(excursion ~ cluster_2, data = data, median)$excursion,
                              count = as.numeric(table(data$cluster_2))), 
                        cbind(aggregate(duration ~ cluster_3, data = data, median), 
                              bandwidth = aggregate(bandwidth ~ cluster_3, data = data, median)$bandwidth, 
                              concavity = aggregate(concavity ~ cluster_3, data = data, median)$concavity, 
                              excursion = aggregate(excursion ~ cluster_3, data = data, median)$excursion,
                              count = as.numeric(table(data$cluster_3))), 
                        cbind(aggregate(duration ~ cluster_4, data = data, median), 
                              bandwidth = aggregate(bandwidth ~ cluster_4, data = data, median)$bandwidth, 
                              concavity = aggregate(concavity ~ cluster_4, data = data, median)$concavity, 
                              excursion = aggregate(excursion ~ cluster_4, data = data, median)$excursion,
                              count = as.numeric(table(data$cluster_4))))
for(x in 1:3){colnames(summarized_data[[x]]) <- c("cluster", "duration", "bandwidth", "concavity", "excursion", "count")}

#collapsed separate data frames into one for plotting
collapsed_data <- cbind(split = c(rep(2, nrow(summarized_data[[1]])),
                                  rep(3, nrow(summarized_data[[2]])),
                                  rep(4, nrow(summarized_data[[3]]))), 
                        do.call(rbind, summarized_data))

#determine whether distributions are statistically different
summary(lm(log(duration) ~ factor(split), data = collapsed_data))

#create legend for plotting
dummy_data <- data.frame(x = rep(1:10, 3), y = rep(1:10, 3), group = rep(c("2", "3", "4"), each = 10))
legend <- ggplot(dummy_data, aes(x = x, y = y, color = group)) + geom_line() + theme_linedraw(base_size = 8, base_family = "Avenir") + 
  scale_color_manual(name = "Deep Split", breaks = c("2", "3", "4"), values = c("#009E73", "#D55E00", "#0072B2")) + 
  theme(legend.position = "right")

#plot distributions for each value of deep split
dist_plot <- ggplot() + 
  #geom_density(data = data, aes(x = duration), linewidth = 0.5, linetype = "dashed") + 
  geom_density(data = collapsed_data, aes(x = duration, group = split, color = factor(split)), linewidth = 0.5) + 
  scale_color_manual(name = "Deep Split", breaks = c("2", "3", "4"), values = c("#009E73", "#D55E00", "#0072B2")) + 
  scale_y_continuous(breaks = c(0, 0.008), limits = c(0, 0.009)) + 
  scale_x_continuous(expand = c(0, 0), limits = c(0, 700)) + 
  theme_linedraw(base_size = 8, base_family = "Avenir") + 
  xlab("Median Duration (ms)") + ylab("Density") + 
  theme(legend.position = "none")

#structure data to look at how median duration changes with syllable reassignment to more granular clusters
reassign_data <- data.frame(med_dur = c(summarized_data[[1]]$duration[match(data$cluster_2, summarized_data[[1]]$cluster)],
                                        summarized_data[[2]]$duration[match(data$cluster_3, summarized_data[[2]]$cluster)],
                                        summarized_data[[3]]$duration[match(data$cluster_4, summarized_data[[3]]$cluster)]),
                            syl_id = rep(1:nrow(data), 3),
                            cluster = rep(2:4, each = nrow(data)))

#determine whether median duration is biased by granularity
reassign_model <- lme4::lmer(log(med_dur) ~ cluster + (1|syl_id), reassign_data)

#restructure to have confidence intervals and estimated effect of increase in cluster on duration (ms) of assigned syllabe
reassign_effect <- c(summary(reassign_model)$coef[2, ], confint(reassign_model)[4, ], Est_in_ms = exp(summary(reassign_model)$coef[2, 1]))

#plot median durations over clusters
reassign_plot <- ggplot(reassign_data, aes(x = cluster, y = med_dur, group = syl_id)) + 
  geom_line(color = scales::alpha("black", 0.01), linewidth = 0.2) + 
  geom_line(aes(y = median(med_dur)), color = "#D55E00", linetype = "dashed") + 
  theme_linedraw(base_size = 8, base_family = "Avenir") + 
  xlab("Deep Split") + ylab("Med. Duration (ms) of Types\n(for Individual Tokens)") + 
  scale_x_continuous(breaks = c(2, 3, 4), labels = c(2, 3, 4), expand = c(0, 0)) + 
  scale_y_continuous(limits = c(0, 800)) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())

#combine and save plot
png("plots/explore_durations.png", width = 6.8, height = 2.4, units = "in", res = 600)
plot_grid(plot_grid(dist_plot, get_legend(legend), rel_widths = c(1, 0.2)), reassign_plot, labels = c("A", "B"), rel_widths = c(1, 0.8))
dev.off()
```
...
where $f(r)$ is the normalized frequency at each rank $r$, $c$ is a normalization parameter, and $\alpha$ and $\beta$ are parameters that control slope and convexity (respectively). According to @izsak06, the bounds of \@ref(eq:zipf-rf-a) are $\alpha > 1$, $\beta > -1$, and $c > 1$. When $\beta = 0$, this function simplifies to the original form of Zipf's rank-frequency law: $f(r) \propto 1/r^\alpha$.
...
```{r figure-zipf-rank-small, echo = FALSE, fig.align = "center", out.width = "50%", fig.cap = "The relationship between rank (x-axis) and count (y-axis) at the intermediate granularity level (DS = 3, 596 syllable types). The blue and orange lines denote the expected distributions according to Zipf's rank-frequency law (blue; a = 1.00, c = 0.08) and Mandelbrot's extension of it (orange; a = 1.20, b = 5.69, c = 0.52)."}
...
As there is no established hypothesis test for Zipf's rank-frequency law, I will simply report the goodness-of-fit (the norm in linguistics) and refer to it as "consistent" with the law when it is within the range reported for human languages ($R^2$ > 0.8) [@piantadosi14; @linders_louwerse23].

The observed frequency distribution is consistent with the Zipf-Mandelbrot distribution at all three levels of granularity in syllable clustering ($R^2 = \{`r signif(zipf_rf_models$r2[4], digits = 3)`, `r signif(zipf_rf_models$r2[5], digits = 3)`, `r signif(zipf_rf_models$r2[6], digits = 3)`\}$ at $DS = \{2, 3, 4\}$), but is inconsistent with the the original form of Zipf's rank frequency law at the two of the three levels of granularity ($R^2 = \{`r signif(zipf_rf_models$r2[1], digits = 3)`, `r signif(zipf_rf_models$r2[2], digits = 3)`, `r signif(zipf_rf_models$r2[3], digits = 3)`\}$). Figure \@ref(fig:figure-zipf-rank-small) shows the rank-frequency distribution at the intermediate granularity level ($DS = 3$; 596 syllable types), while all three can be seen in the [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html). <!--$R^2$ is the most commonly reported goodness-of-fit measure for both forms of Zipf's rank-frequency law, and is generally $> 0.8$ in studies of human language [@linders_louwerse23].--> 

The Zipf-Mandelbrot distribution also outcompetes Zipf's original form and has $R^2 > `r min(signif(c(zipf_rf_models_year[[1]]$r2[4:6], zipf_rf_models_year[[2]]$r2[4:6], zipf_rf_models_year[[3]]$r2[4:6]), digits = 2))`$ when models are fit separately to the data from each year (1975, 2012, and 2019; see [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html)).
...
zipf_plot_a <- cowplot::plot_grid(ggplot(data = syl_types[[3]], aes(x = duration, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                  ...
                  ggplot(data = syl_types[[4]], aes(x = duration, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                     geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                   ...
                   ggplot(data = syl_types[[5]], aes(x = duration, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                     geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
...
zipf_plot_b <- cowplot::plot_grid(ggplot(data = syl_types[[3]], aes(x = concavity, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        ggplot(data = syl_types[[4]], aes(x = concavity, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("Median Concavity (#/ms)"),
                        ggplot(data = syl_types[[5]], aes(x = concavity, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        nrow = 1)

#create zipfian distributions for excursion across three values of deep split
zipf_plot_c <- cowplot::plot_grid(ggplot(data = syl_types[[3]], aes(x = bandwidth, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        ggplot(data = syl_types[[4]], aes(x = bandwidth, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("Median Bandwidth (Hz)"),
                        ggplot(data = syl_types[[5]], aes(x = bandwidth, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        nrow = 1)

#create zipfian distributions for bandwidth across three values of deep split
zipf_plot_d <- cowplot::plot_grid(ggplot(data = syl_types[[3]], aes(x = excursion, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        ggplot(data = syl_types[[4]], aes(x = excursion, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                        ...
                        ggplot(data = syl_types[[5]], aes(x = excursion, y = count)) + geom_point(color = "black", size = 1, stroke = 0) + 
                          geom_smooth(method = "glm", method.args = list(family = "poisson"), color = "#D55E00", linewidth = 0.5) + scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
...
```{r zipf-abbr-tokens-plot, echo = FALSE, eval = FALSE}
#load library
library(ggplot2)
...
#set color of points
point_color <- scales::alpha("black", 0.1)

#create zipfian distributions for duration across three values of deep split
zipf_plot_a <- cowplot::plot_grid(ggplot(data = data, aes(x = duration, y = count_2)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                     scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                     theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("") + labs(subtitle = paste0("Deep Split: 2 (", prettyNum(nrow(syl_types[[3]]), big.mark = ","), " Syllable Types)")),
                  ggplot(data = data, aes(x = duration, y = count_3)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                     scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                     theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("Median Duration (ms)") + labs(subtitle = paste0("Deep Split: 3 (", prettyNum(nrow(syl_types[[4]]), big.mark = ","), " Syllable Types)")),
                   ggplot(data = data, aes(x = duration, y = count_4)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                     scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                     theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("") + labs(subtitle = paste0("Deep Split: 4 (", prettyNum(nrow(syl_types[[5]]), big.mark = ","), " Syllable Types)")),
                   nrow = 1)

#create zipfian distributions for concavity across three values of deep split
zipf_plot_b <- cowplot::plot_grid(ggplot(data = data, aes(x = concavity, y = count_2)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        ggplot(data = data, aes(x = concavity, y = count_3)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("Median Concavity (#/ms)"),
                        ggplot(data = data, aes(x = concavity, y = count_4)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        nrow = 1)

#create zipfian distributions for excursion across three values of deep split
zipf_plot_c <- cowplot::plot_grid(ggplot(data = data, aes(x = bandwidth, y = count_2)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        ggplot(data = data, aes(x = bandwidth, y = count_3)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("Median Bandwidth (Hz)"),
                        ggplot(data = data, aes(x = bandwidth, y = count_4)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        nrow = 1)

#create zipfian distributions for bandwidth across three values of deep split
zipf_plot_d <- cowplot::plot_grid(ggplot(data = data, aes(x = excursion, y = count_2)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        ggplot(data = data, aes(x = excursion, y = count_3)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab("Median Excursion (Hz/ms)"),
                        ggplot(data = data, aes(x = excursion, y = count_4)) + geom_point(color = point_color, size = 1, stroke = 0) + 
                          scale_y_continuous(labels = scales::label_number(suffix = "k", scale = 1e-3)) + 
                          theme_linedraw(base_size = 8, base_family = "Avenir") + ylab(NULL) + xlab(""),
                        nrow = 1)
...
#save plot to file
png("plots/token_dists.png", width = 6.8, height = 4.4, units = "in", res = 600)
cowplot::plot_grid(ggplot() + theme_void(), y_axis, cowplot::plot_grid(zipf_plot_a, zipf_plot_c, zipf_plot_b, zipf_plot_d, nrow = 4, rel_heights = c(1.12, 1, 1, 1)), ncol = 3, rel_widths = c(0.02, 0.003, 1))
dev.off()
```
...
#calculation and significance test for kendall's tau using the ZLAvian package
#mean used rather than mixed because of convergence issues for some conditions
#vignette here: https://cran.r-project.org/web/packages/ZLAvian/vignettes/ZLAvian.html
zla_duration_2 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_2), duration = as.numeric(data$duration), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_duration_3 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_3), duration = as.numeric(data$duration), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_duration_4 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_4), duration = as.numeric(data$duration), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_bandwidth_2 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_2), duration = as.numeric(data$bandwidth), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_bandwidth_3 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_3), duration = as.numeric(data$bandwidth), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_bandwidth_4 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_4), duration = as.numeric(data$bandwidth), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_concavity_2 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_2), duration = as.numeric(data$concavity+0.0001), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_concavity_3 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_3), duration = as.numeric(data$concavity+0.0001), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_concavity_4 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_4), duration = as.numeric(data$concavity+0.0001), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_excursion_2 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_2), duration = as.numeric(data$excursion+0.0001), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_excursion_3 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_3), duration = as.numeric(data$excursion+0.0001), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)
zla_excursion_4 <- ZLAvian::testZLA(data.frame(note = as.character(data$cluster_4), duration = as.numeric(data$excursion+0.0001), ID = factor(data$individual)), minimum = 1, null = 1000, est = "mean", cores = 7)

#construct table of results
zla_kendall_table <- rbind(
  cbind(param = "duration", split = rep(c(2, 3, 4), each = 2), level = c("individual", "population"), as.data.frame(rbind(zla_duration_2$stats, zla_duration_3$stats, zla_duration_4$stats))),
  cbind(param = "bandwidth", split = rep(c(2, 3, 4), each = 2), level = c("individual", "population"), as.data.frame(rbind(zla_bandwidth_2$stats, zla_bandwidth_3$stats, zla_bandwidth_4$stats))),
  cbind(param = "concavity", split = rep(c(2, 3, 4), each = 2), level = c("individual", "population"), as.data.frame(rbind(zla_concavity_2$stats, zla_concavity_3$stats, zla_concavity_4$stats))),
  cbind(param = "excursion", split = rep(c(2, 3, 4), each = 2), level = c("individual", "population"), as.data.frame(rbind(zla_excursion_2$stats, zla_excursion_3$stats, zla_excursion_4$stats)))
)
rownames(zla_kendall_table) <- NULL
zla_kendall_table$sig <- ""
zla_kendall_table$sig[which(zla_kendall_table$p < 0.05)] <- "*"
zla_kendall_table$sig[which(zla_kendall_table$p < 0.01)] <- "**"
zla_kendall_table$sig[which(zla_kendall_table$p < 0.001)] <- "***"
zla_kendall_table$sig[which(zla_kendall_table$p < 0.0001)] <- "****"

#save it
save(zla_kendall_table, file = "data_models/zla_kendall_table.RData")
```

```{r, echo = FALSE, eval = FALSE}
#load library and data
library(brms)
mann_data <- read.csv("data_models/mann_et_al_2021.csv")

#subset mann et al 2021 data into the most heavily sampled house finch-tutored and canary-tutored birds
hofi_tutored_data <- mann_data[which(mann_data$individual == "B1-74"), ]
cnry_tutored_data <- mann_data[which(mann_data$individual == "B1"), ]
no_tutored_data <- mann_data[which(mann_data$individual == "D4"), ]

#add counts of each syllable type and divide concavity by duration to match existing analysis
hofi_tutored_data$count <- sapply(1:nrow(hofi_tutored_data), function(x){length(which(hofi_tutored_data$syllable == hofi_tutored_data$syllable[x]))})
cnry_tutored_data$count <- sapply(1:nrow(cnry_tutored_data), function(x){length(which(cnry_tutored_data$syllable == cnry_tutored_data$syllable[x]))})
no_tutored_data$count <- sapply(1:nrow(no_tutored_data), function(x){length(which(no_tutored_data$syllable == no_tutored_data$syllable[x]))})
hofi_tutored_data$concavity <- hofi_tutored_data$concavity/hofi_tutored_data$duration
cnry_tutored_data$concavity <- cnry_tutored_data$concavity/cnry_tutored_data$duration
no_tutored_data$concavity <- no_tutored_data$concavity/no_tutored_data$duration
...
#run duration, bandwidth, and concavity models for house finch-tutored bird
hofi_tutored_duration_model <- brm(log(duration) ~ scale(count) + (1|syllable), data = hofi_tutored_data, prior = zipf_priors_a, iter = 5000, cores = 4)
hofi_tutored_bandwidth_model <- brm(log(range) ~ scale(count) + (1|syllable), data = hofi_tutored_data, prior = zipf_priors_a, iter = 5000, cores = 4)
hofi_tutored_concavity_model <- brm(log(concavity + 0.0001) ~ scale(count) + (1|syllable), data = hofi_tutored_data, prior = zipf_priors_b, iter = 5000, cores = 4)
hofi_tutored_excursion_model <- brm(log(fex + 0.0001) ~ scale(count) + (1|syllable), data = hofi_tutored_data, prior = zipf_priors_b, iter = 5000, cores = 4)

#run duration, bandwidth, and concavity models for canary-tutored bird
cnry_tutored_duration_model <- brm(log(duration) ~ scale(count) + (1|syllable), data = cnry_tutored_data, prior = zipf_priors_a, iter = 5000, cores = 4)
cnry_tutored_bandwidth_model <- brm(log(range + 1) ~ scale(count) + (1|syllable), data = cnry_tutored_data, prior = zipf_priors_a, iter = 5000, cores = 4) #adding 1 because the minimum bandwidth is 0, which is impossible
cnry_tutored_concavity_model <- brm(log(concavity + 0.0001) ~ scale(count) + (1|syllable), data = cnry_tutored_data, prior = zipf_priors_b, iter = 5000, cores = 4)
cnry_tutored_excursion_model <- brm(log(fex + 0.0001) ~ scale(count) + (1|syllable), data = cnry_tutored_data, prior = zipf_priors_b, iter = 5000, cores = 4)

#run duration, bandwidth, and concavity models for house finch-tutored bird
no_tutored_duration_model <- brm(log(duration) ~ scale(count) + (1|syllable), data = no_tutored_data, prior = zipf_priors_a, iter = 5000, cores = 4)
no_tutored_bandwidth_model <- brm(log(range) ~ scale(count) + (1|syllable), data = no_tutored_data, prior = zipf_priors_a, iter = 5000, cores = 4)
no_tutored_concavity_model <- brm(log(concavity + 0.0001) ~ scale(count) + (1|syllable), data = no_tutored_data, prior = zipf_priors_b, iter = 5000, cores = 4)
no_tutored_excursion_model <- brm(log(fex + 0.0001) ~ scale(count) + (1|syllable), data = no_tutored_data, prior = zipf_priors_b, iter = 5000, cores = 4)

#compute kendall's tau for duration, bandwidth, and concavity models for house-finch tutored bird
hofi_tutored_duration_tau <- ZLAvian::testZLA(data.frame(note = as.character(hofi_tutored_data$syllable), duration = as.numeric(hofi_tutored_data$duration), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
hofi_tutored_bandwidth_tau <- ZLAvian::testZLA(data.frame(note = as.character(hofi_tutored_data$syllable), duration = as.numeric(hofi_tutored_data$range), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
hofi_tutored_concavity_tau <- ZLAvian::testZLA(data.frame(note = as.character(hofi_tutored_data$syllable), duration = as.numeric(hofi_tutored_data$concavity + 0.0001), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
hofi_tutored_excursion_tau <- ZLAvian::testZLA(data.frame(note = as.character(hofi_tutored_data$syllable), duration = as.numeric(hofi_tutored_data$fex + 0.0001), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)

#compute kendall's tau for duration, bandwidth, and concavity models for canary tutored bird
cnry_tutored_duration_tau <- ZLAvian::testZLA(data.frame(note = as.character(cnry_tutored_data$syllable), duration = as.numeric(cnry_tutored_data$duration), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
cnry_tutored_bandwidth_tau <- ZLAvian::testZLA(data.frame(note = as.character(cnry_tutored_data$syllable), duration = as.numeric(cnry_tutored_data$range + 1), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
cnry_tutored_concavity_tau <- ZLAvian::testZLA(data.frame(note = as.character(cnry_tutored_data$syllable), duration = as.numeric(cnry_tutored_data$concavity + 0.0001), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
cnry_tutored_excursion_tau <- ZLAvian::testZLA(data.frame(note = as.character(cnry_tutored_data$syllable), duration = as.numeric(cnry_tutored_data$fex + 0.0001), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)

#compute kendall's tau for duration, bandwidth, and concavity models for bird without tutor
no_tutored_duration_tau <- ZLAvian::testZLA(data.frame(note = as.character(no_tutored_data$syllable), duration = as.numeric(no_tutored_data$duration), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
no_tutored_bandwidth_tau <- ZLAvian::testZLA(data.frame(note = as.character(no_tutored_data$syllable), duration = as.numeric(no_tutored_data$range), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
no_tutored_concavity_tau <- ZLAvian::testZLA(data.frame(note = as.character(no_tutored_data$syllable), duration = as.numeric(no_tutored_data$concavity + 0.0001), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)
no_tutored_excursion_tau <- ZLAvian::testZLA(data.frame(note = as.character(no_tutored_data$syllable), duration = as.numeric(no_tutored_data$fex + 0.0001), ID = factor(1)), minimum = 1, null = 1000, est = "mean", cores = 7)

#combine into object that contains model estimates, diagnostics, and prior specifications, and then save
mann_zipf_replication <- list(list(duration = rbind(summary(hofi_tutored_duration_model)$fixed[2, 1:4], summary(cnry_tutored_duration_model)$fixed[2, 1:4], summary(no_tutored_duration_model)$fixed[2, 1:4]),
                                   bandwidth = rbind(summary(hofi_tutored_bandwidth_model)$fixed[2, 1:4], summary(cnry_tutored_bandwidth_model)$fixed[2, 1:4], summary(no_tutored_bandwidth_model)$fixed[2, 1:4]),
                                   concavity = rbind(summary(hofi_tutored_concavity_model)$fixed[2, 1:4], summary(cnry_tutored_concavity_model)$fixed[2, 1:4], summary(no_tutored_concavity_model)$fixed[2, 1:4]),
                                   excursion = rbind(summary(hofi_tutored_excursion_model)$fixed[2, 1:4], summary(cnry_tutored_excursion_model)$fixed[2, 1:4], summary(no_tutored_excursion_model)$fixed[2, 1:4])),
                              list(duration = rbind(hofi_tutored_duration_tau$stats[1, ], cnry_tutored_duration_tau$stats[1, ], no_tutored_duration_tau$stats[1, ]),
                                   bandwidth = rbind(hofi_tutored_bandwidth_tau$stats[1, ], cnry_tutored_bandwidth_tau$stats[1, ], no_tutored_bandwidth_tau$stats[1, ]),
                                   concavity = rbind(hofi_tutored_concavity_tau$stats[1, ], cnry_tutored_concavity_tau$stats[1, ], no_tutored_concavity_tau$stats[1, ]),
                                   excursion = rbind(hofi_tutored_excursion_tau$stats[1, ], cnry_tutored_excursion_tau$stats[1, ], no_tutored_excursion_tau$stats[1, ])))
save(mann_zipf_replication, file = "data_models/mann_zipf_replication.RData")

#median number of songs from individuals in the youngblood and lahti (2022) dataset, for comparison
#median(sapply(unique(data$individual), function(x){length(unique(data$song[which(data$individual == x)]))}))
```

```{r, eval = FALSE, echo = FALSE}
#CHECKING WHETHER ZLA RESULTS ARE ROBUST TO (1) LONG TAIL AND (2) LOGNORMALITY ASSUMPTION
#get range of values to explore for each parameter (middle 90%)
range_bounds <- c(0.05, 0.95)
duration_range <- quantile(data$duration, probs = c(range_bounds[1], range_bounds[2]))
bandwidth_range <- quantile(data$bandwidth, probs = c(range_bounds[1], range_bounds[2]))
concavity_range <- quantile(data$concavity, probs = c(range_bounds[1], range_bounds[2]))
excursion_range <- quantile(data$excursion, probs = c(range_bounds[1], range_bounds[2]))

#create function for extracting effect and confidence intervals from model
extraction <- function(model){
  temp_a <- lme4::fixef(model)[2]
  temp_b <- confint(model)[4, ]
  return(as.numeric(c(temp_a, temp_b)))
}

#run duration model across all three deep split values
duration_model_2 <- extraction(lme4::lmer(log(duration) ~ scale(count_2) + (1|cluster_2), data = data[which(data$duration > duration_range[1] & data$duration < duration_range[2]), ]))
duration_model_3 <- extraction(lme4::lmer(log(duration) ~ scale(count_3) + (1|cluster_3), data = data[which(data$duration > duration_range[1] & data$duration < duration_range[2]), ]))
duration_model_4 <- extraction(lme4::lmer(log(duration) ~ scale(count_4) + (1|cluster_4), data = data[which(data$duration > duration_range[1] & data$duration < duration_range[2]), ]))

#run bandwidth model across all three deep split values
bandwidth_model_2 <- extraction(lme4::lmer(log(bandwidth) ~ scale(count_2) + (1|cluster_2), data = data[which(data$bandwidth > bandwidth_range[1] & data$bandwidth < bandwidth_range[2]), ]))
bandwidth_model_3 <- extraction(lme4::lmer(log(bandwidth) ~ scale(count_3) + (1|cluster_3), data = data[which(data$bandwidth > bandwidth_range[1] & data$bandwidth < bandwidth_range[2]), ]))
bandwidth_model_4 <- extraction(lme4::lmer(log(bandwidth) ~ scale(count_4) + (1|cluster_4), data = data[which(data$bandwidth > bandwidth_range[1] & data$bandwidth < bandwidth_range[2]), ]))

#run concavity model across all three deep split values, with 0.0001 added to avoid log-transformation issues
concavity_model_2 <- extraction(lme4::lmer(log(concavity + 0.0001) ~ scale(count_2) + (1|cluster_2), data = data[which(data$concavity > concavity_range[1] & data$concavity < concavity_range[2]), ]))
concavity_model_3 <- extraction(lme4::lmer(log(concavity + 0.0001) ~ scale(count_3) + (1|cluster_3), data = data[which(data$concavity > concavity_range[1] & data$concavity < concavity_range[2]), ]))
concavity_model_4 <- extraction(lme4::lmer(log(concavity + 0.0001) ~ scale(count_4) + (1|cluster_4), data = data[which(data$concavity > concavity_range[1] & data$concavity < concavity_range[2]), ]))

#run excursion model across all three deep split values, with 0.0001 added to avoid log-transformation issues
excursion_model_2 <- extraction(lme4::lmer(log(excursion + 0.0001) ~ scale(count_2) + (1|cluster_2), data = data[which(data$excursion > excursion_range[1] & data$excursion < excursion_range[2]), ]))
excursion_model_3 <- extraction(lme4::lmer(log(excursion + 0.0001) ~ scale(count_3) + (1|cluster_3), data = data[which(data$excursion > excursion_range[1] & data$excursion < excursion_range[2]), ]))
excursion_model_4 <- extraction(lme4::lmer(log(excursion + 0.0001) ~ scale(count_4) + (1|cluster_4), data = data[which(data$excursion > excursion_range[1] & data$excursion < excursion_range[2]), ]))

#combine into object that contains median estimate and bounds of 95% CI for main effect of each model at each level of deep split
zipf_models_check_90 <- list(duration = rbind(duration_model_2, duration_model_3, duration_model_4),
                             bandwidth = rbind(bandwidth_model_2, bandwidth_model_3, bandwidth_model_4),
                             concavity = rbind(concavity_model_2, concavity_model_3, concavity_model_4),
                             excursion = rbind(excursion_model_2, excursion_model_3, excursion_model_4))
save(zipf_models_check_90, file = "data_models/zipf_models_check_90.RData")

#run duration model across all three deep split values
duration_model_2 <- extraction(lme4::lmer(scale(duration) ~ scale(count_2) + (1|cluster_2), data = data))
duration_model_3 <- extraction(lme4::lmer(scale(duration) ~ scale(count_3) + (1|cluster_3), data = data))
duration_model_4 <- extraction(lme4::lmer(scale(duration) ~ scale(count_4) + (1|cluster_4), data = data))

#run bandwidth model across all three deep split values
bandwidth_model_2 <- extraction(lme4::lmer(scale(bandwidth) ~ scale(count_2) + (1|cluster_2), data = data))
bandwidth_model_3 <- extraction(lme4::lmer(scale(bandwidth) ~ scale(count_3) + (1|cluster_3), data = data))
bandwidth_model_4 <- extraction(lme4::lmer(scale(bandwidth) ~ scale(count_4) + (1|cluster_4), data = data))

#run concavity model across all three deep split values
concavity_model_2 <- extraction(lme4::lmer(scale(concavity) ~ scale(count_2) + (1|cluster_2), data = data))
concavity_model_3 <- extraction(lme4::lmer(scale(concavity) ~ scale(count_3) + (1|cluster_3), data = data))
concavity_model_4 <- extraction(lme4::lmer(scale(concavity) ~ scale(count_4) + (1|cluster_4), data = data))

#run excursion model across all three deep split values
excursion_model_2 <- extraction(lme4::lmer(scale(excursion) ~ scale(count_2) + (1|cluster_2), data = data))
excursion_model_3 <- extraction(lme4::lmer(scale(excursion) ~ scale(count_3) + (1|cluster_3), data = data))
excursion_model_4 <- extraction(lme4::lmer(scale(excursion) ~ scale(count_4) + (1|cluster_4), data = data))

#combine into object that contains median estimate and bounds of 95% CI for main effect of each model at each level of deep split
zipf_models_check_lognormal <- list(duration = rbind(duration_model_2, duration_model_3, duration_model_4),
                                    bandwidth = rbind(bandwidth_model_2, bandwidth_model_3, bandwidth_model_4),
                                    concavity = rbind(concavity_model_2, concavity_model_3, concavity_model_4),
                                    excursion = rbind(excursion_model_2, excursion_model_3, excursion_model_4))
save(zipf_models_check_lognormal, file = "data_models/zipf_models_check_lognormal.RData")
```

Duration, bandwidth, and excursion had strong negative effects on count at all three levels of granularity in syllable clustering (Table \@ref(tab:zipf-abbr-tables)). Concavity had no effect on count at the first two levels of granularity, but had a very weak negative effect at the third level of granularity. These patterns are apparent in the plots of production costs and counts in the [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html). The results are qualitatively identical when the year of recording (1975, 2012, or 2019) is included as a varying intercept (see [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html)). Several other robustness checks, including replications of this analysis using the rank-based method from the R package ZLAvian [@lewis_etal23; @gilman_etal23] and using manually-classified syllables from an experimental study in house finches [@Mann2020], can be found in the [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html).

## Menzerath's Law

Menzerath's law predicts that longer sequences will be comprised of smaller items. Importantly, Menzerath's law is sometimes detected in random sequences from null models [@g_torre_etal21; @tanaka-ishii21; @ferrer-i-cancho_etal14]. There are two sorts of null models that make sense in this context: (1) random sequences with the same number of syllables as real songs, and (2) pseudorandom sequences that match the cumulative duration in time but can vary in the number of syllables. @jamesPhylogenyMechanismsShared2021 interpret the latter as approximating simple motor constraints---Menzerath's law resulting from efficiency in production alone---where stronger effects in the real data would indicate additional mechanisms (e.g. communicative efficiency). In this study, I compare the real data against each of these to assess both the presence of Menzerath's law and whether it is beyond what would be expected from production constraints.
...
menz_models_year <- cbind(lme4::fixef(menz_models_year), confint(menz_models_year)[4:5, ])
...
```{r, echo = FALSE}
#compute are of overlap between the production constraint model and the actual data
#https://stackoverflow.com/questions/64882496/how-to-calculate-the-overlap-between-2-dataset-distribution
dens_range <- range(c(menz_models$actual$samples, menz_models$prod_null$samples))
actual_dens <- density(menz_models$actual$samples, from = dens_range[1], to = dens_range[2])
prod_dens <- density(menz_models$prod_null$samples, from = dens_range[1], to = dens_range[2])
joint_dens <- pmin(actual_dens$y, prod_dens$y)
overlap <- paste0(round((sum(joint_dens)/sum(actual_dens$y, prod_dens$y))*100, digits = 1), "%")
```

The results of the log-normal model indicate that song length has a negative effect on syllable duration (Mean Estimate: `r signif(menz_models$actual$estimates[2, 1], digits = 2)`; 95% CI: [`r signif(menz_models$actual$estimates[2, 3], digits = 2)`, `r signif(menz_models$actual$estimates[2, 4], digits = 2)`]) (left panel of Figure \@ref(fig:figure-menzerath)). The posterior distribution for this effect from the model fit to the actual data (orange) is more negative than the posterior distributions from 10 simulated datasets from the production constraint null model of @jamesPhylogenyMechanismsShared2021 (green) and a simple random null model (blue), although there is notable overlap between the lower tail of the production constraint model and the upper tail of the actual data (`r overlap` of the combined distribution area; right panel of Figure \@ref(fig:figure-menzerath)). The negative effect of song length on syllable duration persists when the year of recording (1975, 2012, or 2019) is included as a varying intercept (see [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html)).
...
Figure \@ref(fig:figure-small-world)B shows the distribution of $SWI$ calculated from random sequences with the same distribution of types (orange) and the actual sequences (blue). At all three levels of granularity in syllable clustering, the observed small-worldness of the real songs is above both the standard threshold ($SWI > 1$) and the pseudorandom sequences. $SWI > 1$ when computed separately from the data from each year (1975, 2012, and 2019; see [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html))
...
```{r mutual-information-song-lengths, eval = FALSE, echo = FALSE}
#load packages
library(brms)
...
#calculate mutual information across three levels of deep split
mi_data_song_lengths <- parallel::mclapply(1:3, function(i){
  #use appropriate syllables
  if(i == 1){data$syls <- data$cluster_2}
  if(i == 2){data$syls <- data$cluster_3}
  if(i == 3){data$syls <- data$cluster_4}
  ...
  #compute actual mutual information curves
  actual <- sapply(1:(max(song_lengths) - 1), function(m){
    #set m as the distance between syllables in the sequence
    dist <- m

    #store pairs within bouts
    pairs <- do.call(rbind, lapply(unique_songs[which(song_lengths > m)], function(r){
      seq <- data$syls[which(data$song == r)]
      t(sapply(1:(length(seq)-dist), function(x){c(seq[x], seq[x+dist])}))
    }))
  ...
  #compute shuffled mutual information curves
  shuffled <- sapply(1:(max(song_lengths) - 1), function(m){
    #set m as the distance between syllables in the sequence
    dist <- m

    #store pairs within bouts
    pairs <- do.call(rbind, lapply(unique_songs[which(song_lengths > m)], function(r){
      seq <- sample(data$syls[which(data$song == r)])
      t(sapply(1:(length(seq)-dist), function(x){c(seq[x], seq[x+dist])}))
    }))
...
#run models across the three years at all three levels of deep split
mi_models_song_lengths <- do.call(rbind, lapply(1:3, function(g){
  #fit the exponential model
  exponential_fit <- brm(bf(mi ~ a*exp((-dist)*b), a + b ~ 1, nl = TRUE),
                         data = mi_data_song_lengths[[g]][1:100, ], prior = exponential_priors,
                         iter = 5000, cores = 4)
  
  #fit the power law model
  power_law_fit <- brm(bf(mi ~ c*(dist^(-d)), c + d ~ 1, nl = TRUE),
                       data = mi_data_song_lengths[[g]][1:100, ], prior = power_law_priors, 
                       iter = 5000, cores = 4)
  
  #fit the composite model
  composite_fit <- brm(bf(mi ~ a*exp((-dist)*b) + c*(dist^(-d)), a + b + c + d ~ 1, nl = TRUE), 
                       data = mi_data_song_lengths[[g]][1:100, ], prior = composite_priors, 
                       iter = 5000, cores = 4)
...
#format data frame
mi_models_song_lengths <- as.data.frame(mi_models_song_lengths)
mi_models_song_lengths <- cbind(c(2, 3, 4), mi_models_song_lengths)
colnames(mi_models_song_lengths) <- c("DS", "Exponential", "Power-Law", "Composite")

#save object
save(mi_models_song_lengths, file = "data_models/mi_models_song_lengths.RData")
```

```{r mutual-information-bout-level, eval = FALSE, echo = FALSE}
#load packages
library(brms)
...
#shuffle up syllables within songs
data <- do.call(rbind, parallel::mclapply(unique_songs, function(x){
  #store data from individual
  temp <- data[which(data$song == x), ]
...
#calculate mutual information across three levels of deep split
mi_data_shuff_syls <- parallel::mclapply(1:3, function(i){
  #use appropriate syllables
  if(i == 1){data$syls <- data$cluster_2}
  if(i == 2){data$syls <- data$cluster_3}
  if(i == 3){data$syls <- data$cluster_4}
...
#shuffle up songs within each bout
data <- do.call(rbind, parallel::mclapply(unique_inds, function(x){
  #store data from individual
  temp <- data[which(data$individual == x), ]

  #get random vector of songs and reorder temp accordingly
  rand_song_order <- sample(unique(temp$song))
  temp <- temp[unlist(lapply(rand_song_order, function(y){which(temp$song %in% y)})), ]
...
#calculate mutual information across three levels of deep split
mi_data_shuff_both <- parallel::mclapply(1:3, function(i){
  #use appropriate syllables
  if(i == 1){data$syls <- data$cluster_2}
  if(i == 2){data$syls <- data$cluster_3}
  if(i == 3){data$syls <- data$cluster_4}
...
#run wilcox and t-tests to see whether info is greater when bout structure is kept
mi_models_bout_level <- list(wilcox = lapply(1:3, function(x){wilcox.test(mi_data_shuff_syls[[x]]$mi, 
                                                                          mi_data_shuff_both[[x]]$mi, paired = TRUE, alternative = "greater")}),
                             ttest = lapply(1:3, function(x){t.test(mi_data_shuff_syls[[x]]$mi, 
                                                                    mi_data_shuff_both[[x]]$mi, paired = TRUE, alternative = "greater")}))

#save object
save(mi_models_bout_level, file = "data_models/mi_models_bout_level.RData")
```
...
Interestingly, the transitions in the composite curves from Figure \@ref(fig:figure-mutual-info) roughly correspond to the average length of a house finch song (~12 syllables) [@Ju2019]. I reran the analysis with individual song sequences rather than song bouts and found that the exponential model outcompeted the composite model at $DS = \{3, 4\}$ (see [Supplementary Information](https://masonyoungblood.github.io/linguistic_efficiency/supplement/supplement.html)). This suggests that individual song sequences may have Markovian structure within hierarchically-organized song bouts.
...
This work was inspired in large part by conversations with Olivier Morin, Alexey Koshevoy, and the attendees of the Communicative Efficiency Workshop hosted by the Max Planck Institute for Geoanthropology in April 2023. <!-- I would also like to thank Dan Mann for sharing his manually-classified syllable data for supplemental analysis [@Mann2020]. -->

# Data & Code Availability {-}

The data for this study come from @youngbloodContentBiasCultural2022 (https://github.com/masonyoungblood/TransmissionBias). The analysis code for this study is available in the interactive version of this manuscript (https://masonyoungblood.github.io/linguistic_efficiency) and the source file in the corresponding GitHub (https://github.com/masonyoungblood/linguistic_efficiency) and Zenodo repositories (DOI).
